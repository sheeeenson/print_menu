...
export function A3PosterSection({ project }) {
  ...
  return <section className="app-page a3-poster-page">
    <aside className="app-side-panel a3-poster-side-panel">
      ...
      <section className="app-control-group"><div className="panel-title-row"><h3>Products</h3><button type="button" onClick={autoArrangeProducts}>Auto arrange</button></div><div className="a3-template-row">{[1,2,3,4,5].map((count) => <button key={count} type="button" className={selectedPoster.productCount === count ? 'active' : ''} onClick={() => handleProductCount(count)}>{count}</button>)}</div><small>Each image, title, description and price is now a separate object. Click and drag any element directly on the poster; use the round handle to resize only that element.</small></section>
    ...
    <main className="app-preview-stage a3-poster-preview-stage"><div className="app-toolbar"><div><p>Preview</p><h2>{selectedPoster.name}</h2></div><div className="app-pill">{getA3Format(selectedPoster.formatId).label}</div></div><A3PosterPreview poster={selectedPoster} dishes={dishes} onCommitStroke={commitStroke} onEraseStrokes={eraseStrokes} onUpdateElementTransform={updateElementTransform} onSelectElement={selectElement} onUpdateOfferTransform={updateOfferTransform} /></main>
  </section>;
}

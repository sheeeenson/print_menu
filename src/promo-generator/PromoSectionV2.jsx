  ...
  return (
    <div className="promo-gif-library">
      ...
      <div className="promo-gif-library-actions">
        ...
        {gifLibrary.length ? <select value={selectedLibraryUrl} onChange={(event) => event.target.value && updateSettings({ gifUrl: event.target.value })}><option value="">Choose saved GIF</option>{gifLibrary.map((item) => <option key={item.id} value={item.url}>{item.name}</option>)}</select> : null}
      </div>
      {gifLibrary.length ? <div className="promo-gif-library-list">{gifLibrary.map((item) => <div key={item.id} className={item.url === gifUrl ? 'active' : ''}><button type="button" onClick={() => updateSettings({ gifUrl: item.url })}><span>{item.name}</span><small>{item.url}</small></button><button type="button" className="danger" onClick={() => deleteGif(item.url)}>Delete</button></div>)}</div> : <small className="promo-preview-size">Saved GIFs will appear here after you save a URL.</small>}
    </div>
  );
}

...
const getPreviewDriveMediaUrl = (fileId) => {
  const route = ['drive', 'media'].join('-');
  return `/api/${route}?id=${encodeURIComponent(fileId)}`;
};
...
export function isVideoLikeUrl(value = '') {
  ...
  if (!url) return false;
  const proxyRoute = ['drive', 'media'].join('-');
  if (url.includes(`/api/${proxyRoute}?id=`)) return true;

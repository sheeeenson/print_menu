...
const getBackgroundDim = (settings, hasMedia) => clamp(settings.backgroundDim ?? (hasMedia ? 0 : 0), 0, 70) / 100;
...
export function SiteBannerPreview({ dish, settings, index = 0 }) {
  ...
  const sceneClass = [
    'site-banner-scene',
    customBackgroundUrl ? 'site-banner-scene-has-background-media site-banner-scene-has-custom-background' : '',
  ].filter(Boolean).join(' ');

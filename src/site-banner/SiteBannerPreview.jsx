...
const getAutoBackgroundUrl = (dish) => normalizeBackgroundImageUrl(isTransparentProduct(dish) ? dish?.promoBackgroundUrl : '');
const getBackgroundFit = (settings) => settings.backgroundFit === 'contain' || settings.backgroundFit === 'fill' ? settings.backgroundFit : 'cover';
const getBackgroundDim = (settings, backgroundMode) => {
  const fallback = backgroundMode === 'auto' ? 28 : 0;
  return clamp(settings.backgroundDim ?? fallback, 0, 70) / 100;
};
...
export function SiteBannerPreview({ dish, settings, index = 0 }) {
  ...
  const backgroundDim = getBackgroundDim(settings, backgroundVariant);
